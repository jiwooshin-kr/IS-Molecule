#!/bin/bash
# MDLM (absorbing-state) counterpart of scripts/server_run_comparison.sh.
# Waits for scripts/server_train_mdlm_qm9.sh and the absorbing-state D-CBG
...
# does not depend on x_0, so `aggregate_x0` and `marginal` coincide exactly; and
# its posterior has no uniform floor, so a token that none of the N candidates
# proposed gets *exactly* zero probability rather than the small floor uniform
# diffusion leaves. That makes it a direct test of whether the atomic
# truncation helps (as the UDLM support_floor results suggest) or hurts.
#
# Usage (inside tmux):
#   bash scripts/server_run_comparison_mdlm.sh
set -uo pipefail
...
echo "######## MDLM + ours: support floor ########"
# Absorbing state zeroes unproposed tokens exactly, so the floor should matter
# more here than it did for UDLM.
for FLOOR in 0.01 0.1; do
  run_eval "mdlm_ours_N500_lam1000_floor${FLOOR}_win0.0-1.0" "${OURS_ARGS[@]}" \
    guidance.num_x0_samples=500 guidance.lambda_=1000 \
    "guidance.support_floor=${FLOOR}" \
    guidance.t_min=0.0 guidance.t_max=1.0
done
