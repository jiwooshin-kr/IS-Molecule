...
#
# Usage (inside tmux):
#   bash scripts/server_run_cbg_exact.sh
set -uo pipefail
...
for GAMMA in 1 2 5; do
  ...
  if python -u -W ignore guidance_eval/our_qm9_eval.py \
      ...
      > "${ROOT}/results/${TAG}.log" 2>&1; then
    grep -E "^\s+Valid|Mean:" "${ROOT}/results/${TAG}.log"
  else
    echo "  FAILED"; tail -4 "${ROOT}/results/${TAG}.log"
  fi
done
echo "######## Done ########"
