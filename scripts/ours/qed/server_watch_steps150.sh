...
# connected. Companion to scripts/ours/qed/server_watch_and_tabulate.sh, which watches
# server_run_comparison.sh instead.
#
#   tmux new-session -d -s agg "bash scripts/ours/qed/server_watch_steps150.sh"
#
...
tabulate_now() {
  python scripts/ours/make_results_table.py "${ROOT}/results/qed" "${ROOT}/results/qed" \
    > "${ROOT}/results/qed/sweep_table.txt" 2>&1
...
PY
  ...
  } > "${ROOT}/results/qed/steps150_summary.txt" 2>&1
}
...
echo "Table:   ${ROOT}/results/qed/sweep_table.{md,tex,txt}"
echo "Summary: ${ROOT}/results/qed/steps150_summary.txt"
echo "Raw logs: ${ROOT}/results/qed/_sweep_st150.out, _sweep_st150_lambda.out"
