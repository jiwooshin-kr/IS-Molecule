...
# after it finishes. Launch alongside scripts/ours/qed/server_run_comparison.sh so the
# aggregated table is always current without anyone having to be connected:
#
#   tmux new-session -d -s agg "bash scripts/ours/qed/server_watch_and_tabulate.sh"
#
...
tabulate_now
echo "Sweep finished; final table in ${ROOT}/results/qed/sweep_table.{md,tex,txt}"
